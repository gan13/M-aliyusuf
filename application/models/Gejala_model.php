...
class Gejala_model extends CI_Model
{
  ...
  function simpan_gejala($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_gejala($data_update, $where, $table_name)
	{
		$this->db->where($where);
	...
	function count_all_search_gejala($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_gejala($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('nama_gejala', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('gejala');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('gejala.display_controler', $q);
		$this->db->or_like('gejala.nama_controler', $q);
		$this->db->limit('20');
		$result = $this->db->get('gejala');
		return $result->result_array();
	}
