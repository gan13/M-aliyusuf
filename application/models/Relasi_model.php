...
class Relasi_model extends CI_Model
{
	...
	public function json_all_relasi($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
  ...
  function simpan_relasi($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_relasi($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_relasi($where) {	
		$this->db->from('relasi');
		$this->db->where($where);
	...
	function count_all_search_relasi($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_relasi($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('id_persyaratan', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('relasi');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('relasi.id_persyaratan', $q);
		$this->db->or_like('relasi.id_persyaratan', $q);
		$this->db->limit('20');
		$result = $this->db->get('relasi');
		return $result->result_array();
	}
