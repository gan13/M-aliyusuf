...
class Diagnosa_model extends CI_Model
{
	...
	public function json_all_diagnosa($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
  ...
  function simpan_diagnosa($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_diagnosa($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_diagnosa($where) {	
	...
	function count_all_search_diagnosa($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_diagnosa($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('nama_diagnosa', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('diagnosa');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('diagnosa.display_controler', $q);
		$this->db->or_like('diagnosa.nama_controler', $q);
		$this->db->limit('20');
		$result = $this->db->get('diagnosa');
		return $result->result_array();
	}
